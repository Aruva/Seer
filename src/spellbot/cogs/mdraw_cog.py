...
class MDrawCog(commands.Cog):
    def __init__(self, bot: SpellBot) -> None:
        self.bot = bot

    @app_commands.command(
        name="mdraw",
        description="Log a Magic: The Gathering 1v1 match draw.",
    )
    ...
    async def mdraw(
        ...
        async with db_session_manager():
            await log_magic_match(self.bot, interaction, format, opponent, is_win=False)


async def setup(bot: SpellBot) -> None:  # pragma: no cover
    await bot.add_cog(MDrawCog(bot), guild=settings.GUILD_OBJECT)
