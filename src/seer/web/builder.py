...
from seer.database import db_session_manager, initialize_connection
from seer.models import import_models
from seer.services import ServicesRegistry
from seer.settings import settings
from seer.web.api import ping, record, rest
...
def build_web_app() -> web.Application:
    ...
    app.add_routes(
        [
            ...
            web.post(r"/api/infraction", rest.infraction_create_endpoint),
            web.get(r"/api/infraction/{guild}/{user}", rest.infraction_list_endpoint),
            web.get(r"/api/player/{guild}/search", rest.player_search_endpoint),
            web.get(r"/api/player/{guild}/{user}", rest.player_summary_endpoint),
        ],
    )
    return app
...
def launch_web_server(loop: Loop, port: int) -> None:  # pragma: no cover
    ...
    loop.run_until_complete(initialize_connection("seer-web"))
