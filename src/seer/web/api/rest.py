...
from seer.services import NotificationData, ServicesRegistry
from seer.settings import settings
from seer.web.tools import rate_limited
...
if TYPE_CHECKING:
    ...
    from seer.models import GameDict, UserDict
        ...
        "embeds": [
            {
                ...
                "footer": {"text": f"SouthSeer Game ID: #SS{game['id']}"},
            },
        ],
    }
        ...
        "embeds": [
            {
                ...
                "footer": {
                    "text": f"SouthSeer Notification ID: #SN{notif.id} — Service: {service}",
                },
            },
        ],
        "components": [
...
@tracer.wrap(name="rest", resource="infraction_create_endpoint")
async def infraction_create_endpoint(request: web.Request) -> WebResponse:
    try:
        async with db_session_manager():
            services = ServicesRegistry()
            payload = await request.json()
            ...
            infraction_type = payload["infraction"]
            infraction_category = payload["infraction_category"]
            penalty = payload["penalty"]
            judge_xid = int(payload["judge_xid"]) if payload.get("judge_xid") else None
            judge_name = payload.get("judge_name")
            round_num = payload.get("round")
            notes = payload.get("notes")
            result = await services.infractions.create(
                guild_xid=guild_xid,
                user_xid=user_xid,
                judge_xid=judge_xid,
            ...
            return reply({"success": True, "infraction": result}, status=201)
    except KeyError as e:
        return reply(error=f"missing key: {e}", status=400)
    except Exception as e:
        return reply(error=str(e))
...
@tracer.wrap(name="rest", resource="infraction_list_endpoint")
async def infraction_list_endpoint(request: web.Request) -> WebResponse:
    try:
        async with db_session_manager():
            services = ServicesRegistry()
            guild_xid = int(request.match_info["guild"])
            user_xid = int(request.match_info["user"])
            result = await services.infractions.get_by_user(
                guild_xid=guild_xid,
                user_xid=user_xid,
            )
            return reply({"infractions": result})
    except Exception as e:
        return reply(error=str(e))


@tracer.wrap(name="rest", resource="player_search_endpoint")
async def player_search_endpoint(request: web.Request) -> WebResponse:
    try:
        async with db_session_manager():
            services = ServicesRegistry()
            guild_xid = int(request.match_info["guild"])
            ...
            if not query or len(query) < 2:
                return reply(error="Query must be at least 2 characters", status=400)
            limit = min(int(request.query.get("limit", 20)), 50)
            result = await services.infractions.search_players(
                guild_xid=guild_xid,
                query=query,
                limit=limit,
            )
            return reply({"players": result})
    except Exception as e:
        return reply(error=str(e))


@tracer.wrap(name="rest", resource="player_summary_endpoint")
async def player_summary_endpoint(request: web.Request) -> WebResponse:
    try:
        async with db_session_manager():
            services = ServicesRegistry()
            guild_xid = int(request.match_info["guild"])
            user_xid = int(request.match_info["user"])
            result = await services.infractions.get_player_summary(
                guild_xid=guild_xid,
                user_xid=user_xid,
            )
            if not result:
                return reply(error="Player not found", status=404)
            return reply({"player": result})
    except Exception as e:
        return reply(error=str(e))
