...
class MagicMatchView(discord.ui.View):
    """Persistent confirmation view for 1v1 Magic: The Gathering matches.

    Uses separate custom_ids from both LeagueMatchView and WargameMatchView
    so all three can coexist without button routing conflicts.
    """
    ...
    @ui.button(
        custom_id="magic_match_confirm",
        label="Confirm",
        emoji="✅",
        style=discord.ButtonStyle.success,
    )
    async def confirm(
        self, interaction: discord.Interaction, button: ui.Button[MagicMatchView]
    ) -> None:
    ...
    async def _handle_dispute(self, interaction: discord.Interaction) -> None:
        ...
        thread = await interaction.message.create_thread(name="Magic Match Dispute")

        from seer.services import WargameConfigService, WargameMatchesService
        svc = WargameMatchesService()
    ...
    async def _apply_elo(self, match) -> dict[int, tuple[int, int, int]]:
        """Apply ELO deltas for a confirmed Magic match."""
        try:
            from seer.services import WargameEloService
            ...
            logger.warning("Failed to apply Magic ELO for match %s: %s", match.id, exc)
            return {}
    ...
    async def _send_confirmation_dms(
        ...
        """DM each player a match summary with ELO change."""
        channel_link = (
        ...
        for player in match.players:
            ...
            if elo_results and player.user_xid in elo_results:
                ...
                embed.add_field(
                    name="🎯 ELO Change",
                    value=f"{arrow} {sign}{delta} → **{new_elo}** ELO *(was {old_elo})*",
                    inline=False,
                )
    ...
    async def _refresh_embed(self, interaction: discord.Interaction, match) -> None:
        assert interaction.message is not None
        try:
            ...
            logger.warning("Failed to refresh magic match embed: %s", exc)
