...
class Game(Base):
    ...
    id = Column(
        ...
        nullable=False,
        primary_key=True,
        doc="The Seer game reference ID",
    )
    created_at = Column(
        DateTime,
        nullable=False,
    ...
    def embed_description_link_info(
        ...
        if effective_service.fallback_url:
            return (
                "Sorry but SouthSeer was unable to create a link"
    ...
    def embed_footer(self) -> str:
        return f"SouthSeer Game ID: #SS{self.id} — Service: {GameService(self.service)}"
