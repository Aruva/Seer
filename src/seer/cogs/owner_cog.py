...
from seer.actions.base_action import handle_exception
from seer.database import db_session_manager
from seer.metrics import add_span_context
from seer.operations import is_bad_user, safe_send_user
from seer.services import ServicesRegistry
from seer.settings import settings
from seer.utils import for_all_callbacks, load_extensions
...
async def set_banned(banned: bool, ctx: commands.Context[Seer], arg: str | None) -> None:
    assert ctx.message
    if arg is None:
...
async def set_banned_guild(banned: bool, ctx: commands.Context[Seer], arg: str | None) -> None:
    assert ctx.message
    if arg is None:
...
class OwnerCog(commands.Cog):
    ...
    async def sync(self, ctx: commands.Context[Seer]) -> None:
    ...
    async def ban(self, ctx: commands.Context[Seer], arg: str | None = None) -> None:
    ...
    async def unban(self, ctx: commands.Context[Seer], arg: str | None = None) -> None:
        add_span_context(ctx)
        async with db_session_manager():
            try:
    ...
    async def ban_guild(self, ctx: commands.Context[Seer], arg: str | None = None) -> None:
        add_span_context(ctx)
        async with db_session_manager():
            try:
    ...
    async def unban_guild(self, ctx: commands.Context[Seer], arg: str | None = None) -> None:
        add_span_context(ctx)
        async with db_session_manager():
            try:
    ...
    async def stats(self, ctx: commands.Context[Seer]) -> None:
        add_span_context(ctx)
        await safe_send_user(
            ctx.message.author,
            cleandoc(
    ...
    async def is_bad(self, ctx: commands.Context[Seer], arg: str | None) -> None:
        add_span_context(ctx)
