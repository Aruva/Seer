...
from seer.models import Channel, ChannelDict, GuildAward, GuildAwardDict
...
from seer.utils import EMBED_DESCRIPTION_SIZE_LIMIT
...
class AdminAction(BaseAction):
    def __init__(self, bot: Seer, interaction: discord.Interaction) -> None:
    ...
    async def _build_awards_embeds(self) -> list[Embed]:
        ...
        def new_embed() -> Embed:
            embed = Embed(title=f"SouthSeer Player Awards for {guild['name']}")
            embed.set_thumbnail(url=settings.ICO_URL)
            embed.color = discord.Color(settings.INFO_EMBED_COLOR)
            return embed
    ...
    async def _build_setup_embed(self) -> Embed:
        guild = await self.services.guilds.to_dict()
        embed = Embed(title=f"SouthSeer Setup for {guild['name']}")
        embed.set_thumbnail(url=settings.ICO_URL)
        description = (
            "These are the current settings for SouthSeer on this server."
            " Please use the buttons below, as well as the `/set` commands,"
            " to setup SouthSeer as you wish.\n\n"
    ...
    async def setup_mythic_track(self) -> None:
        ...
        embed.color = discord.Color(settings.INFO_EMBED_COLOR)
        if enabled:
            GUIDE = "https://www.mythictrack.com/seer"
            embed.description = (
                ...
                "While this feature is turned on, SouthSeer will generate PIN codes for "
