...
from seer.database import DatabaseSession, db_session_manager
...
from seer.metrics import setup_ignored_errors
from seer.services import ServicesRegistry
from seer.utils import user_can_moderate
...
if TYPE_CHECKING:
    ...
    from seer.models import ChannelDict, GuildDict
...
def handle_exception(ex: Exception) -> NoReturn:
    if isinstance(ex, SeerError):  # pragma: no cover
        raise ex
    logger.exception(
...
class BaseAction:
    ...
    async def create(
        ...
        with tracer.trace(name=f"seer.interactions.{cls.__name__}.create") as span:
